...
BACKUP_DIR="_local_patch_backup/api_cache_safety_v2_${STAMP}"
mkdir -p "$BACKUP_DIR/config" "$BACKUP_DIR/core"

for file in config/settings.py core/crypto_market.py; do
    if [[ -f "$file" ]]; then
...
while IFS= read -r file; do
    if grep -Eq "get_subscription_payload|FAILURE_CACHE_SECONDS|청약|subscription" "$file"; then
        mkdir -p "$BACKUP_DIR/$(dirname "$file")"
        cp "$file" "$BACKUP_DIR/$file"
    fi
done < <(find core -type f -name "*.py")
...
def find_subscription_path() -> Path:
    preferred = [
        Path("core/subscription_data.py"),
        Path("core/subscription.py"),
        Path("core/subscription_api.py"),
        Path("core/apt_subscription.py"),
        Path("core/cheongyak.py"),
    ]
    ...
    if not candidates:
        searched = "\n".join(str(p) for p in Path("core").rglob("*.py"))
        raise SystemExit(
            "청약정보 Python 파일을 자동으로 찾지 못했습니다.\n"
            "검색한 파일:\n" + searched
        )

    candidates.sort(key=lambda item: (-item[0], str(item[1])))
    selected = candidates[0][1]
    print(f"청약정보 파일 자동 감지: {selected}")
    return selected

subscription_path = find_subscription_path()
...
"$PYTHON_BIN" -m compileall -q core
