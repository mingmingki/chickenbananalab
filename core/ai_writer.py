...
def cbl_adsense_structure_instruction(writing_style=None, category=None, title=None, language=None):
    """
    애드센스 승인용 글 구조 랜덤 강화 V4
    ...
    핵심:
    - 선택된 구조를 실제 H2 전개 순서로 강제한다.
    - 기존 고정 목차보다 이 랜덤 구조를 우선 적용한다.
    """
    import random

    style_raw = str(writing_style or "").strip()
    category_raw = str(category or "").strip()
    title_raw = str(title or "").strip()
    language_raw = str(language or "").strip()

    structure_bank = {
        "자연 설명형": [
            "초보자가 처음 궁금해하는 질문으로 시작 → 핵심 용어를 쉬운 말로 풀이 → 왜 알아야 하는지 설명 → 실제 사용 상황 예시 → 처음 적용할 때의 기준 → 주의할 점 → 다음에 확인할 내용",
            "독자가 겪는 불편한 상황 제시 → 그 문제가 생기는 이유 → 기본 개념을 짧게 정리 → 해결 방향을 쉽게 설명 → 상황별 예시 → 실수 방지 팁 → 마무리 정리",
            "일상적인 비유로 관심 유도 → 비유와 주제 연결 → 핵심 원리 설명 → 실제 화면이나 행동 기준 설명 → 활용하면 좋은 경우 → 한계 설명 → 결론",
            "검색자가 원하는 답을 먼저 제시 → 답이 그렇게 되는 배경 설명 → 세부 요소를 나누어 설명 → 예시로 다시 이해시키기 → 확인해야 할 체크포인트 → FAQ → 정리",
            "처음 접하는 사람의 시선으로 도입 → 헷갈리는 용어 구분 → 기본 구조 설명 → 실제 적용 순서 → 적용 후 기대할 수 있는 변화 → 조심해야 할 부분 → 최종 요약",
            "짧은 사례로 시작 → 사례 속 문제 찾기 → 문제를 이해하기 위한 개념 설명 → 해결 과정 설명 → 비슷한 상황에서의 적용법 → 놓치기 쉬운 부분 → 결론",
            "왜 사람들이 이 주제를 검색하는지 설명 → 가장 먼저 알아야 할 핵심 정리 → 세부 개념을 단계별로 풀기 → 실제 예시 → 장점보다 중요한 판단 기준 → 마무리",
            "흔한 오해를 먼저 제시 → 오해가 생기는 이유 → 올바른 개념 설명 → 실제 상황에서의 차이 → 잘못 적용했을 때 문제 → 안전한 활용 팁 → 결론",
            "큰 그림부터 설명 → 전체 흐름을 구성 요소로 나누기 → 각 요소의 역할 설명 → 실제 사용 장면 연결 → 독자가 직접 확인할 부분 → FAQ → 정리",
            "문제 상황을 먼저 묘사 → 원인 후보를 나누어 설명 → 가장 중요한 개념 소개 → 단계별 해결 흐름 → 결과 확인 방법 → 추가 팁 → 마무리",
            "질문형 도입 → 짧은 답변 제시 → 답변의 근거 설명 → 예시와 반례 비교 → 언제 유용한지 설명 → 언제 조심해야 하는지 설명 → 최종 정리",
            "처음 알아야 할 배경 설명 → 핵심 개념 하나씩 풀기 → 실제로 많이 쓰이는 상황 → 초보자가 헷갈리는 부분 → 쉽게 기억하는 방법 → 결론",
            "현재 상황이나 트렌드로 시작 → 왜 관심이 커졌는지 설명 → 기본 원리 설명 → 생활 또는 업무 적용 사례 → 장점과 한계 → 다음 행동 가이드 → 마무리",
            "가장 쉬운 정의부터 제시 → 비슷한 개념과 구분 → 작동 방식 설명 → 실제 예시 → 선택 또는 활용 기준 → 주의사항 → 요약",
            "독자의 고민을 한 문장으로 정리 → 그 고민이 생기는 배경 → 핵심 개념 설명 → 선택 가능한 방법들 → 각 방법의 차이 → 추천 기준 → 결론",
            "Before 상황 설명 → After 변화 설명 → 그 차이를 만드는 핵심 개념 → 적용 방법 → 적용 전 확인사항 → 적용 후 점검사항 → 정리",
            "짧은 이야기식 도입 → 이야기 속 핵심 문제 → 개념 설명 → 해결 또는 활용 과정 → 독자가 따라 할 수 있는 단계 → 주의할 점 → 마무리",
            "초보자용 핵심 요약 → 조금 더 자세한 원리 설명 → 상황별 적용 예시 → 실전에서 자주 하는 실수 → 실수를 줄이는 기준 → FAQ → 결론",
            "가장 많이 묻는 질문으로 시작 → 질문에 대한 쉬운 답 → 답을 이해하기 위한 배경 → 실제 예시 → 추가로 알아둘 점 → 최종 정리",
            "실제 업무 또는 생활 장면 도입 → 그 장면에서 필요한 이유 → 기본 개념 설명 → 활용 순서 → 효율이 좋아지는 지점 → 한계와 주의점 → 결론",
            "용어부터 차근차근 정리 → 관련 개념 연결 → 실제 작동 흐름 설명 → 사용자가 체감하는 차이 → 확인 방법 → 마무리",
            "문제 원인을 먼저 나누기 → 원인별로 쉽게 설명 → 핵심 해결 원리 → 실전 적용 단계 → 해결되지 않을 때 점검할 것 → 결론",
            "비슷한 선택지 제시 → 각각을 쉽게 설명 → 차이가 나는 기준 → 상황별 추천 → 선택 전 주의사항 → 최종 판단",
            "체크해야 할 상황 제시 → 기본 개념 설명 → 첫 번째 확인 기준 → 두 번째 확인 기준 → 실제 예시 → 마지막 점검 → 정리",
            "기초 개념을 한눈에 정리 → 왜 중요한지 설명 → 어떻게 활용되는지 설명 → 실제 사례 → 잘못 이해하기 쉬운 부분 → FAQ → 마무리",
            "독자가 당장 필요한 답 제시 → 답을 적용하기 전 알아야 할 것 → 단계별 적용 방법 → 결과 확인법 → 자주 생기는 문제 → 결론",
            "쉬운 비유로 핵심 설명 → 비유의 한계 설명 → 실제 개념으로 전환 → 세부 요소 설명 → 활용 사례 → 주의사항 → 정리",
            "처음에는 몰라도 되는 것과 꼭 알아야 할 것 구분 → 꼭 알아야 할 핵심 설명 → 나중에 알아도 되는 세부 내용 → 실제 활용 → 마무리",
            "상황별 시나리오 제시 → 시나리오별 필요한 개념 → 적용 방법 차이 → 가장 현실적인 선택 기준 → 추가 팁 → 결론",
            "전체 요약 먼저 제시 → 핵심 포인트 3가지 설명 → 각 포인트의 실제 의미 → 적용 사례 → 독자가 확인할 체크리스트 → 최종 정리",
        ],
        "전문가 분석형": [
            "핵심 결론 선제시 → 시장 또는 기술 배경 → 주요 변수 분석 → 장점과 기회 → 리스크와 한계 → 실무적 판단 기준 → 전망",
            "현재 흐름 진단 → 핵심 쟁점 도출 → 원인 분석 → 영향 범위 설명 → 대응 전략 → 향후 관찰 포인트 → 결론",
            "분석 기준 제시 → 항목별 평가 → 실제 영향 분석 → 반대 관점 검토 → 종합 판단 → 실무 제언",
            "개념 정의 → 구조적 원인 분석 → 비용과 효율 관점 → 리스크 관리 → 적용 조건 → 최종 정리",
            "주요 변화 요약 → 배경 요인 → 이해관계자 영향 → 기회 요인 → 위험 요인 → 전망",
            "핵심 이슈 도출 → 비교 기준 설정 → 세부 항목 분석 → 실무 적용성 → 한계와 보완점 → 결론",
            "전문가 관점 도입 → 핵심 지표 설명 → 변수별 영향 → 예상 시나리오 → 대응 방향 → 정리",
            "문제의 본질 → 구조 분석 → 단기 영향 → 중장기 영향 → 전략적 시사점 → 마무리",
            "산업 맥락 설명 → 핵심 트렌드 → 기술 또는 제도 분석 → 파급 효과 → 실무 체크포인트 → 전망",
            "팩트 정리 → 해석 기준 → 핵심 변수 → 시나리오별 전망 → 의사결정 포인트 → 결론",
        ],
        "경험 기반형": [
            "직접 겪을 법한 상황 도입 → 문제 인식 → 시도 과정 → 느낀 점 → 개선 팁 → 결론",
            "처음 접한 계기 → 기대했던 점 → 실제 경험 → 좋았던 점 → 아쉬운 점 → 정리",
            "현장감 있는 사례 → 당시 문제 → 해결 과정 → 배운 점 → 독자에게 줄 팁 → 마무리",
            "개인적 관찰 도입 → 핵심 경험 설명 → 세부 장면 → 판단 기준 → 추천 또는 주의사항 → 결론",
            "실패 또는 시행착오 → 원인 분석 → 다시 적용한 방법 → 결과 → 체크포인트 → 정리",
            "처음에는 몰랐던 점 → 경험 후 알게 된 점 → 실제 차이 → 활용 팁 → 마무리",
            "구체적 상황 묘사 → 선택 이유 → 진행 과정 → 결과 비교 → 느낀 점 → 결론",
            "경험 전 기대 → 경험 중 발견한 문제 → 해결 방법 → 얻은 효과 → 주의할 점 → 정리",
            "실제 사례 중심 도입 → 핵심 포인트 → 세부 과정 → 장점과 한계 → 독자 조언 → 마무리",
            "짧은 에피소드 → 핵심 교훈 → 적용 방법 → 비슷한 상황의 팁 → 최종 정리",
        ],
        "구매·리뷰형": [
            "구매 전 고민 → 핵심 특징 → 실제 사용감 → 장점 → 단점 → 추천 대상 → 결론",
            "선택 이유 → 첫인상 → 주요 기능 → 체감 성능 → 아쉬운 부분 → 구매 판단 기준 → 정리",
            "비교 대상 제시 → 핵심 차이 → 가격과 효율 → 사용 후기 → 추천 상황 → 마무리",
            "구매 목적 → 기대 포인트 → 실제 만족도 → 불편한 점 → 대안 비교 → 결론",
            "상품 개요 → 주요 스펙 또는 특징 → 사용 장면 → 장단점 → 누구에게 맞는지 → 정리",
            "처음 본 느낌 → 사용 전 확인할 점 → 실제 활용 → 유지 관리 포인트 → 최종 평가",
            "가격대 설명 → 핵심 기능 → 실사용 장점 → 단점 보완 방법 → 구매 전 체크리스트 → 결론",
            "왜 관심을 받는지 → 특징 분석 → 사용 후기식 설명 → 경쟁 제품과 차이 → 추천 여부",
            "구매 전 체크사항 → 선택 기준 → 실제 사용성 → 만족 포인트 → 주의사항 → 마무리",
            "한 줄 평가 → 세부 평가 → 장점 → 단점 → 추천 대상 → 비추천 대상 → 최종 정리",
        ],
        "뉴스·트렌드형": [
            "이슈 요약 → 발생 배경 → 현재 상황 → 주요 쟁점 → 영향 분석 → 향후 전망 → 정리",
            "최근 변화 소개 → 왜 주목받는지 → 핵심 내용 → 시장 또는 사용자 영향 → 체크 포인트 → 결론",
            "뉴스 핵심 정리 → 관련 배경 → 이해관계자 반응 → 리스크와 기회 → 전망",
            "트렌드 도입 → 변화의 원인 → 주요 사례 → 파급 효과 → 앞으로 볼 점 → 마무리",
            "오늘의 흐름 → 핵심 키워드 → 세부 내용 → 의미 분석 → 독자가 알아둘 점 → 결론",
            "최근 이슈 제시 → 이전 흐름과 비교 → 현재 변화 → 예상 영향 → 주의할 변수 → 정리",
            "화제가 된 이유 → 핵심 내용 → 관련 산업 또는 생활 영향 → 장단점 → 전망",
            "트렌드 배경 → 주요 변화 → 대표 사례 → 실질적 의미 → 향후 체크리스트 → 마무리",
            "짧은 뉴스 요약 → 깊이 있는 배경 설명 → 쟁점 분석 → 대응 방향 → 결론",
            "현재 상황 정리 → 핵심 원인 → 단기 전망 → 중장기 가능성 → 독자 행동 가이드 → 정리",
        ],
        "체크리스트형": [
            "핵심 개념 → 사전 확인사항 → 체크리스트 → 놓치기 쉬운 부분 → 실전 팁 → 결론",
            "왜 점검해야 하는지 → 기본 기준 → 항목별 체크 → 문제 발생 시 대처 → 정리",
            "준비 단계 → 실행 단계 → 검토 단계 → 주의사항 → 최종 체크 → 마무리",
            "처음 확인할 것 → 반드시 봐야 할 것 → 비교해야 할 것 → 피해야 할 실수 → 결론",
            "상황별 체크 기준 → 세부 항목 → 우선순위 → 실제 적용 팁 → 정리",
            "핵심 요약 → 1차 체크 항목 → 2차 체크 항목 → 위험 신호 → 마무리",
            "초보자 기준 점검 → 중급자 기준 점검 → 실무 기준 점검 → 추가 팁 → 결론",
            "필수 확인사항 → 선택 확인사항 → 비용 시간 효율 체크 → 주의할 점 → 정리",
            "문제 예방 관점 → 점검 리스트 → 흔한 실수 → 보완 방법 → 최종 정리",
            "빠른 체크 → 상세 체크 → 상황별 판단 기준 → 실행 후 검토 → 마무리",
        ],
    }

    def detect_style_key(raw):
        raw = str(raw or "").strip()
        compact = raw.lower().replace(" ", "")

        if "전문" in raw or "분석" in raw or "expert" in compact or "analysis" in compact:
            return "전문가 분석형"
        if "경험" in raw or "experience" in compact:
            return "경험 기반형"
        if "구매" in raw or "리뷰" in raw or "review" in compact or "purchase" in compact:
            return "구매·리뷰형"
        if "뉴스" in raw or "트렌드" in raw or "news" in compact or "trend" in compact:
            return "뉴스·트렌드형"
        if "체크" in raw or "checklist" in compact:
            return "체크리스트형"
        return "자연 설명형"

    style_key = detect_style_key(style_raw)
    selected_structure = random.choice(structure_bank[style_key])

    structure_steps = [x.strip() for x in selected_structure.split("→") if x.strip()]
    h2_blueprint_text = "\n".join([f"{idx}. {step}" for idx, step in enumerate(structure_steps, start=1)])

    opening_rules = {
        "자연 설명형": [
            "처음부터 딱딱한 정의로 시작하지 말고, 독자가 실제로 겪을 수 있는 상황에서 자연스럽게 시작한다.",
            "검색자가 왜 이 주제를 찾아보게 되었는지 공감하는 문장으로 도입한다.",
            "일상이나 업무에서 마주칠 수 있는 사례를 먼저 제시하고 본문으로 연결한다.",
        ],
        "전문가 분석형": [
            "단순 소개가 아니라 분석 관점과 판단 기준을 먼저 잡고 시작한다.",
            "표면적인 장점보다 실제 의사결정에 영향을 주는 변수부터 설명한다.",
            "시장, 기술, 비용, 효율, 리스크 중 주제에 맞는 분석축을 설정한다.",
        ],
        "경험 기반형": [
            "실제 경험담처럼 보이되 과장하지 말고, 구체적인 상황과 판단 과정을 자연스럽게 담는다.",
            "개인적인 관찰, 시행착오, 느낀 점을 섞어 사람이 쓴 글처럼 전개한다.",
        ],
        "구매·리뷰형": [
            "구매 전 고민, 실제 사용감, 장단점, 추천 대상을 균형 있게 설명한다.",
            "광고 문구처럼 과장하지 말고 실사용 리뷰처럼 구체적으로 작성한다.",
        ],
        "뉴스·트렌드형": [
            "최근 흐름을 먼저 요약하고, 배경과 의미를 독자가 이해할 수 있게 풀어쓴다.",
            "단순 뉴스 전달이 아니라 왜 중요한 변화인지까지 설명한다.",
        ],
        "체크리스트형": [
            "독자가 바로 확인할 수 있는 점검 기준을 중심으로 구성한다.",
            "단순 목록만 나열하지 말고 각 체크 항목의 이유와 주의점을 함께 설명한다.",
        ],
    }

    paragraph_rules = [
        "각 H2 아래에는 최소 2개 이상의 문단을 작성한다.",
        "각 소제목 아래에는 3~5문장 이상의 자연스러운 설명형 문단을 포함한다.",
        "문단마다 배경, 원인, 예시, 판단 기준 중 최소 2가지를 섞어 설명한다.",
        "단순 목록 나열로 끝내지 말고 독자가 이해할 수 있도록 맥락을 충분히 풀어쓴다.",
    ]

    anti_pattern_rules = [
        "매번 같은 '개념 설명 → 장점 → 활용 방법 → 주의사항 → FAQ → 결론' 순서를 반복하지 않는다.",
        "소제목 이름을 기계적으로 반복하지 말고 글 주제에 맞게 자연스럽게 바꾼다.",
        "AI가 쓴 것처럼 보이는 과한 요약문, 반복 문장, 뻔한 결론을 피한다.",
        "본문 첫 문장을 '오늘은 ~ 알아보겠습니다' 식으로 반복하지 않는다.",
    ]

    depth_rules = [
        "본문은 최소 6개 이상의 의미 있는 소제목으로 구성한다.",
        "도입과 결론을 제외하고 최소 5개 이상의 본문 섹션을 둔다.",
        "핵심 설명, 사례, 주의점, 체크포인트가 모두 포함되도록 충분히 길게 작성한다.",
        "얕은 설명으로 끝내지 말고 검색자가 머무를 만한 깊이 있는 문단을 만든다.",
    ]

    faq_count = random.randint(2, 4)

    return f"""

[CBL_ADSENSE_STRUCTURE_RANDOMIZER]
아래 지시는 글 작성 품질을 높이기 위한 최상위 작성 규칙이다. 이 블록 제목이나 규칙 문구를 본문에 그대로 출력하지 마라.

중요:
- 기존 프롬프트에 고정 목차, 고정 소제목, 반복적인 SEO 목차가 있더라도 이번 글은 반드시 아래의 랜덤 목차 설계를 우선 적용한다.
- 매번 같은 '개념 설명 → 장점 → 활용 방법 → 주의사항 → FAQ → 결론' 구조로 작성하지 않는다.
- 아래 세부 구조의 각 단계를 실제 본문 H2 소제목 흐름으로 변환해서 작성한다.
- H2 소제목 문구는 아래 단계명을 그대로 복사하지 말고, 글 제목에 맞게 자연스러운 소제목으로 새로 만든다.
- H2 순서와 전개 방식은 반드시 이번 세부 구조의 순서를 따른다.

사용자가 선택한 글 스타일: {style_key}
이번 글의 세부 구조 원본:
{selected_structure}
...
글 제목/주제 참고: {title_raw}
카테고리 참고: {category_raw}
기존 선택 스타일 참고: {style_raw}
언어 참고: {language_raw}

작성 방식:
- {random.choice(opening_rules.get(style_key, opening_rules["자연 설명형"]))}
- {random.choice(paragraph_rules)}
- {random.choice(anti_pattern_rules)}
- {random.choice(depth_rules)}
- 도입부는 너무 짧게 끝내지 말고, 독자가 왜 이 글을 읽어야 하는지 자연스럽게 설명한다.
- 본문은 단순 정보 나열이 아니라 배경 설명, 예시, 판단 기준, 주의사항을 섞어 풍부하게 작성한다.
- 각 H2는 서로 다른 역할을 가져야 하며, 같은 의미의 소제목을 반복하지 않는다.
- 결론은 요약만 하지 말고 독자가 다음에 무엇을 확인하면 좋은지까지 자연스럽게 마무리한다.
- FAQ는 정확히 {faq_count}개만 작성한다.
- FAQ 질문은 매번 비슷한 질문을 반복하지 말고, 검색자가 실제로 궁금해할 만한 질문으로 구성한다.
- 광고 승인용 저품질 문장처럼 보이지 않도록 의미 없는 반복 문장과 지나치게 짧은 문단을 피한다.
